...
const Slug = ({addtoCart,product,variants,buyNow}) => {
  console.log(product,variants)
    ...
    const checkServiceability = async()=>{
      ...
      {
        setvalidpin(true)
        toast.success('Your pin code is serviceble', {
        ...
        setvalidpin(false)
        toast.error('Your pin code is not serviceble', {
          position: "bottom-center",
          autoClose: 1000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        ...
        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
          <div className="flex">
            ...
            {Object.keys(variants).includes('green') && Object.keys(variants['green']).includes(size) && <button onClick={()=>{refreshVariant(size,'green')}} className={`border-2 border-gray-300 ml-1 bg-white-700 rounded-full w-6 h-6 focus:outline-none ${color==='green' ? 'border-black':'border-gray-300'}`}></button> }
            {Object.keys(variants).includes('blue') && Object.keys(variants['blue']).includes(size) && <button onClick={()=>{refreshVariant(size,'blue')}} className={`border-2 border-gray-300 ml-1 bg-white-700 rounded-full w-6 h-6 focus:outline-none ${color==='blue' ? 'border-black':'border-gray-300'}`}></button> }
            {Object.keys(variants).includes('purple') && Object.keys(variants['purple']).includes(size) && <button onClick={()=>{refreshVariant(size,'purple')}} className={`border-2 border-gray-300 ml-1 bg-white-700 rounded-full w-6 h-6 focus:outline-none ${color==='purple' ? 'border-black':'border-gray-300'}`}></button> }
        ...
        <div className="flex flex-wrap md:flex-nowrap">
          ...
          <button onClick={()=>{addtoCart(slug,1,product.price,product.title,size,color)}} className="flex  sm:ml-8 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded disabled:opacity-25" disabled={validpin==false}>Add to Cart</button>
          <button onClick={()=>{buyNow(slug,1,product.price,product.title,size,color)}} className="flex  sm:ml-4 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded">Buy Now</button>
          </div>
...
export async function getServerSideProps(context) {
  ...
  let colorSizeSlug={}  //{red:{xl:{slug:'wear-the-colde-xl'}}
